...
function correlate(submissions) {
  ...
  const fsx=submissions.flatMap(s=>s.files.map(f=>({...f,supplier:s.supplier,zipName:s.zipName})));
  for(let i=0;i<fsx.length;i++)for(let j=i+1;j<fsx.length;j++){const a=fsx[i],b=fsx[j];if(a.supplier===b.supplier)continue;
    if(a.sha256===b.sha256)add('相同文件',a.sha256,a,b,95,'不同供应商提交了内容完全一致的文件',a.zipPath,b.zipPath);
    ...
    for(const [k,t,w] of pairs){const x=normalize(a.attributes[k]),y=normalize(b.attributes[k]);if(x&&x===y)add(t,a.attributes[k],a,b,w,`不同供应商文件的${t.replace('相同','')}一致`,a.zipPath,b.zipPath);}
    for(const k of ['documentIds','localPaths','externalLinks','mediaHashes']){const common=(a.attributes[k]||[]).map(normalize).filter(Boolean).filter(x=>(b.attributes[k]||[]).map(normalize).includes(x));for(const x of common.slice(0,5))add(k==='documentIds'?'相同文档ID':k==='localPaths'?'相同本地路径':k==='mediaHashes'?'相同图片':'相同外部链接',x,a,b,k==='documentIds'||k==='localPaths'?90:k==='mediaHashes'?80:55,`发现较具唯一性的共同${k}`,a.zipPath,b.zipPath);}
    const d=hamming(a.attributes.textFingerprint,b.attributes.textFingerprint);if(d<=3&&a.attributes.textFingerprint)add('文本高度相似',`${a.attributes.textFingerprint} / 距离 ${d}`,a,b,55,'本地 SimHash 显示文本结构高度相似；需排除招标方统一内容',a.zipPath,b.zipPath);
  }
