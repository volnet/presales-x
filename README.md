...
PreSalesX 是一款默认离线运行的桌面专业工具，面向方案、报价、资质材料和其他售前文档。它提供“多家供应商批量审查”“文件属性编辑”和“文件脱敏”三个独立工作台，并配有可拖动、可互动的桌面数字伙伴。
...
- PDF 支持文档属性、批注、Watermark/Stamp Annotation 水印，以及 Word 等程序导出后形成的低透明度大号旋转文字或大型图形水印；无法可靠区分正文的普通页面内容不会被破坏性删除。
- 导出时使用“另存为”选择具体文件并可直接修改文件名；绝不覆盖原文件，宏与其他未处理的包内容原样保留。

### 桌面数字伙伴

数字伙伴以约原设计 2/3 的尺寸悬浮在桌面边缘，只显示人物本身，不显示名称、提示语或气泡。直接拖动人物即可移动，点击切换动作，双击聚焦主窗口；它会持续做轻微的呼吸、漂浮动作并更频繁地切换情绪。关闭 PreSalesX 主窗口时，数字伙伴会随程序一同退出。
...
```text
dist\PreSalesX-1.3.0-Windows-x64.zip
```
...
输出为 `dist/PreSalesX-1.3.0-macOS-universal.zip`，同时包含 Intel 与 Apple Silicon 所需运行代码。解压后运行 `PreSalesX.app`，无需另行安装 Node.js 或 npm 依赖。未配置 Apple Developer 签名时，首次启动可能需要在 Finder 中右键选择“打开”。

推送 `v1.3.0` 标签后，GitHub Actions 会分别在 Windows 和 macOS 构建机上生成上述两个 ZIP，并自动附加到 GitHub Release。
