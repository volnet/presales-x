...
const {inspectOfficeImages,loadOfficeMedia,replaceOfficeMedia,kindFor,dimensions}=require('../src/office-images');
...
test('detects Word, Excel and PowerPoint image media without Office automation',async()=>{
  ...
  try{
    ...
    assert.ok(result.every(item=>item.indexMode==='lazy'));
    assert.ok(result.every(item=>item.images[0].data===undefined));
    for(const item of result){const loaded=await loadOfficeMedia({sourcePath:item.sourcePath,entryPath:item.images[0].entryPath});assert.equal(loaded.width,1);assert.equal(loaded.height,1);assert.equal(loaded.data,png.toString('base64'));}
...
test('extracts embedded Office video with media metadata',async()=>{
  ...
  try{await officePackage(source,'ppt/media/media1.mp4',Buffer.from('fake-mp4'));const [result]=await inspectOfficeImages([source]),media=result.images[0];assert.equal(media.mediaType,'video');assert.equal(media.ext,'mp4');assert.equal(media.mime,'video/mp4');assert.equal(media.previewable,true);assert.equal(media.processable,false);assert.equal(media.data,undefined);const loaded=await loadOfficeMedia({sourcePath:source,entryPath:media.entryPath});assert.equal((await fs.readFile(loaded.path)).toString(),'fake-mp4');}finally{await fs.rm(directory,{recursive:true,force:true});}
});
